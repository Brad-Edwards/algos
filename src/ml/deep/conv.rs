...
impl Conv2D {
    ...
    pub fn forward(&self, input: &Vec<Vec<Vec<Vec<f64>>>>) -> (Vec<Vec<Vec<Vec<f64>>>>, Conv2DCache) {
        ...
        let mut output = vec![vec![vec![vec![0.0; output_width]; output_height]; self.out_channels]; batch_size];
    ...
    pub fn backward(&self, grad_output: &Vec<Vec<Vec<Vec<f64>>>>, cache: &Conv2DCache) 
        -> (Vec<Vec<Vec<Vec<f64>>>>, Conv2DGradients) 
    {
        ...
        let mut grad_input = vec![vec![vec![vec![0.0; cache.input[0][0][0].len()]; 
                                           cache.input[0][0].len()]; 
                                           self.in_channels]; 
                                           batch_size];
        let mut grad_weights = vec![vec![vec![vec![0.0; self.kernel_size.1]; 
                                                 self.kernel_size.0]; 
                                                 self.in_channels]; 
                                                 self.out_channels];
                ...
                for h in 0..output_height {
                    for w in 0..output_width {
                        ...
                        for in_c in 0..self.in_channels {
                            for kh in 0..self.kernel_size.0 {
                                for kw in 0..self.kernel_size.1 {
                                    grad_weights[out_c][in_c][kh][kw] += 
                                        cache.padded_input[b][in_c][h_start + kh][w_start + kw] *
                                        grad_output[b][out_c][h][w];
                                        
                                    // Gradient with respect to input
                                    if h_start + kh < grad_input[0][0].len() && 
                                       w_start + kw < grad_input[0][0][0].len() {
                                        grad_input[b][in_c][h_start + kh][w_start + kw] +=
                                            self.weights[out_c][in_c][kh][kw] *
                                            grad_output[b][out_c][h][w];
                                    }
                                }
    ...
    fn pad_input(&self, input: &Vec<Vec<Vec<Vec<f64>>>>) -> Vec<Vec<Vec<Vec<f64>>>> {
        let batch_size = input.len();
        let input_height = input[0][0].len();
        let input_width = input[0][0][0].len();
        ...
        let mut padded = vec![vec![vec![vec![0.0; padded_width]; padded_height]; 
                                      self.in_channels]; 
                                      batch_size];
...
mod tests {
    ...
    fn test_forward() {
        ...
        assert_eq!(output[0].len(), 64); // out channels
        assert_eq!(output[0][0].len(), 32); // height
        assert_eq!(output[0][0][0].len(), 32); // width

...
pub fn bwt_transform(input: &[u8]) -> (Vec<u8>, usize) {
    ...
    let transformed: Vec<u8> = rotations.iter().map(|rot| rot[n - 1]).collect();
...
pub fn bwt_transform_suffix_array(input: &[u8]) -> (Vec<u8>, usize) {
    ...
    let mut suffixes: Vec<(usize, &[u8])> =
        (0..n).map(|i| (i, &circular_input[i..i + n])).collect();
    ...
    let transformed: Vec<u8> = suffixes
        .iter()
        .map(|(i, _)| {
    ...
    let orig_index = suffixes.iter().position(|(i, _)| *i == 0).unwrap_or(0);
...
mod tests {
    ...
    fn test_bwt_round_trip() {
        ...
        let examples = [b"banana".to_vec(), b"abracadabra".to_vec()];
...
pub fn run_length_decode(encoded: &[u8]) -> Vec<u8> {
    ...
    while i < encoded.len() {
        ...
        if i + 3 < encoded.len() && encoded[i] == encoded[i + 1] && encoded[i] == encoded[i + 2] {
            // It's a run
